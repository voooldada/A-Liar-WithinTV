...
const tvOpenSiteButton = document.getElementById('tv-open-tv-site-button');
...
const translations = {
  pt: {
    ...
    recruitmentMessage: 'Você foi convidado para se unir ao Traidor. Aceita?',
    accept: 'Aceitar',
    decline: 'Recusar',
  ...
  en: {
    ...
    recruitmentMessage: 'You have been invited to join the Traitor. Accept?',
    accept: 'Accept',
    decline: 'Decline',
...
function showDeathNotification(deadPlayer, role, options = {}) {
  ...
  if (deathPlayerRole) {
    deathPlayerRole.textContent = gameRoles.getRoleName(role);
...
// Abrir site da TV já com o código da sala
if (tvOpenSiteButton) {
  tvOpenSiteButton.addEventListener('click', (e) => {
    e.preventDefault();
    if (!currentRoom) return setTVScannerStatus('Crie ou entre numa sala primeiro.', 'error');
    ...
    case PHASES.DAY:
      {
        ...
        try {
          if (previousPhase !== PHASES.VOTING) {
            setupDayAudio();
            if (audioPermissionGranted && dayAudio) {
              playOverlayAudio(dayAudio, dayMusic, 0.15);
            }
          } else {
            console.log('⏭️ Skip Day narration after voting phase');
          }
        } catch (err) {
...
function setupGameListeners() {
  ...
  continueActionsUnsubscribe = gameVoting.listenContinueActions(async (actions) => {
    ...
    if (gameVoting.hasAllContinued(aliveIds)) {
      ...
      console.log('✅ All continued, night cycle begins');
      if (continueButton) continueButton.disabled = false;
      await gameTurns.setPhase(PHASES.NIGHT);
    }
  });
...
async function resolveVoting() {
  ...
  try {
    ...
    if (eliminated) {
      ...
      // Show death sequence
      showDeathSequence(eliminatedPlayer, eliminatedRole, eliminated, () => {
        // After sequence, show notification
        showDeathNotification(eliminatedPlayer, eliminatedRole, { skipEliminationAudio: eliminatedRole === ROLES.ASSASSIN });

        // Limpar votos
        gameVoting.clearVotes();
        ...
        update(roomRef, rolesUpdate);

        // Aguardar notificação antes de avançar
        setTimeout(async () => {
          console.log('⏭️ Advancing after voting death');
          // Reset flag before advancing
          ...
          if (assassinKilledByVote && traitorAlive) {
            const announcement = 'O Assassino foi eliminado, mas ainda existe um Traidor na cidade. O jogo continua até que ele seja descoberto.';
            try {
              await gameState.updateGameState({ dayAnnouncement: announcement });
            } catch (error) {
              console.warn('⚠️ Não foi possível salvar a mensagem do dia após eliminação do Assassino:', error);
            }
          }
